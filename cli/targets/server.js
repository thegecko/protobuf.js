...
server_target.description = "Static server code without reflection (non-functional on its own)";

function server_target(root, options, callback) {
    config = options;
    try {
        var aliases = [];
        if (config.decode)
...
function buildService(ref, service) {
    ...
    push("function " + escapeName(service.name) + "(requestDelimited, responseDelimited) {");
    ++indent;
    push("$protobuf.rpc.Server.call(this, requestDelimited, responseDelimited);");
    push("this.methods = {");
    ++indent;
    service.methodsArray.forEach(function(method) {
        var lcName = protobuf.util.lcFirst(method.name);
        push(escapeName(lcName) + ": { req: $root." + exportName(method.resolvedRequestType) + ", res: $root." + exportName(method.resolvedResponseType) + " },");
    });
    --indent;
    push("};");
    --indent;
    push("}");
    push("");
    push("(" + escapeName(service.name) + ".prototype = Object.create($protobuf.rpc.Server.prototype)).constructor = " + escapeName(service.name) + ";");

    if (config.create) {
        push("");
        pushComment([
            "Creates new " + service.name + " server.",
            "@function create",
            "@memberof " + exportName(service),
            "@static",
            "@param {boolean} [requestDelimited=false] Whether requests are length-delimited",
            "@param {boolean} [responseDelimited=false] Whether responses are length-delimited",
            "@returns {" + escapeName(service.name) + "} RPC server. Useful where requests and/or responses are streamed."
        ]);
        push(escapeName(service.name) + ".create = function create(requestDelimited, responseDelimited) {");
            ++indent;
            push("return new this(requestDelimited, responseDelimited);");
            --indent;
        push("};");
    }
